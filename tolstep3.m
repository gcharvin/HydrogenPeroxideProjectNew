...
[param,funH,funA,funQ,funR]=tolparam3; % initialize paremeters for WT;
param.nullclines=[];
...
tstart=700; % time of stress onset
...
for j=1:numel(mutants)
    ...
    for i=1:numel(irange)
        ...
        param2(i,j).nullclines= tolanalytics3(0,parama);
        ...
        [T,Y]=tolmodel3(param2(i,j),funH,funA,funQ,funR);
  ...
  %      ylim([0 2]);
      ...
      %  ylim([0 1]);
        ...
        mu(i)= param2(i,j).mu0 * max(0,(1 - Y(end,1) / param.k));
...
plot(T(pix:end)-T(pix),Y(pix:end,2),'LineWidth',2,'Color',col); hold on;
 ...
 mu= param.mu0 * max(0,(1 - Y(:,1) / param.k));
