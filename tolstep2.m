function [outT,outY,options]=tolstep2(display)
...
[param,funH,funA]=tolparam2; % initialize paremeters for WT;
param.nullclines=[];
...
for j=1:numel(mutants)
    ...
    for i=1:numel(irange)
        ...
        param2(i,j).nullclines= tolanalytics2(0,parama);
        ...
        [T,Y]=tolmodel2(param2(i,j),funH,funA);
...
plot(T,Y(:,2),'LineWidth',2,'Color',col); hold on;
 ...
 mu= param.mu0 * (param.k^param.n)./(param.k^param.n + Y(:,1).^param.n);
 plot(T,mu,'LineWidth',2,'Color',col); hold on;
